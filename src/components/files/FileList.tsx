  ...
  if (error) {
    return (
      <div className="card" style={{ borderColor: 'rgba(239,68,68,0.2)', background: 'rgba(127,29,29,0.1)' }}>
        <p className="text-sm text-red-400">{t.fileList.loadError}</p>
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 sm:py-20 text-center">
        <div className="w-20 h-20 rounded-2xl flex items-center justify-center mb-5 animate-float"
          style={{ background: 'rgba(0,125,255,0.06)', border: '1px solid rgba(0,125,255,0.15)' }}
        >
        ...
        <p className="text-sm text-zinc-500 max-w-xs leading-relaxed">{t.fileList.noFilesDesc}</p>
      </div>
    )
  }
