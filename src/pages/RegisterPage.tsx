    ...
    <>
      <style>{`
        ...
        .reg-orb-1 {
          ...
          top: -120px; right: 12%;
          background: radial-gradient(circle, color-mix(in srgb, var(--brand-secondary) 11%, transparent) 0%, transparent 65%);
          animation: reg-drift1 16s ease-in-out infinite;
        }
        .reg-orb-2 {
          width: 280px; height: 280px;
          bottom: -80px; left: 18%;
          background: radial-gradient(circle, color-mix(in srgb, var(--brand-primary) 9%, transparent) 0%, transparent 65%);
          animation: reg-drift2 20s ease-in-out infinite;
        }
        @keyframes reg-drift1 {
          0%, 100% { transform: translateY(0px); }
          50%       { transform: translateY(-22px); }
        }
        @keyframes reg-drift2 {
          0%, 100% { transform: translateY(0px); }
        ...
        .reg-scanline {
          ...
          opacity: 0.5;
          animation: reg-scan 4s ease-in-out infinite;
          z-index: 100;
        }
        @keyframes reg-scan {
          0%, 100% { opacity: 0.3; }
        ...
        .reg-input::placeholder { color: var(--muted); opacity: 0.6; }
        .reg-input:focus {
        ...
        .reg-btn-primary:hover:not(:disabled)::before { opacity: 1; }
        .reg-btn-primary:disabled { opacity: 0.7; cursor: not-allowed; }
        .reg-btn-primary > * { position: relative; z-index: 1; }
        ...
        .reg-btn-google:active { transform: scale(0.985); }
      ...
      <div className="reg-root min-h-[calc(100vh-8rem)] flex items-center justify-center px-4">
        ...
        >
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold">{t.auth.createVault}</h1>
            <p className="mt-2 text-[var(--muted)]">{t.auth.createVaultSubtitle}</p>
          </div>
          ...
          <div className="reg-panel">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label htmlFor="register-email" className="block text-sm font-medium opacity-80 mb-2">{t.auth.email}</label>
                <input
              ...
              <div>
                <label htmlFor="register-password" className="block text-sm font-medium opacity-80 mb-2">{t.auth.password}</label>
                <input
                  id="register-password" type="password" value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder={t.auth.passwordPlaceholder}
                  required minLength={6} className="reg-input"
                />
              </div>

              <div>
                <label htmlFor="confirm-password" className="block text-sm font-medium opacity-80 mb-2">{t.auth.confirmPassword}</label>
                <input
              ...
              <div className="reg-divider">
                <div className="reg-divider-label">
                  <span>{t.auth.orContinueWith}</span>
                </div>
              </div>

              {/* Google */}
              <button type="button" onClick={handleGoogleLogin} className="reg-btn-google">
